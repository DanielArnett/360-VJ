...
static const char fragmentShaderCode[] = R"(#version 410 core
	...
	void main()
	{
		...
		//	return;
		//}
		//float aspectInput = (Brightness.g / textureSize.y);
		...
		//u = int(percentX * ((float(textureSize.x)) / 2.0)) + int(textureSize.x) / 2;
		//v = int(percentY * ((float(textureSize.y)) / 2.0)) + int(textureSize.y) / 2;
